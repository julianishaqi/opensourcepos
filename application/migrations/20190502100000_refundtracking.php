...
class Migration_RefundTracking extends CI_Migration
{
	...
	public function up()
	{
		...
		$this->db->query('CREATE TEMPORARY TABLE IF NOT EXISTS ' . $this->db->dbprefix('migrate_refund') .
			' (INDEX(sale_id))
			(
				select a.sale_id, total_payments - trans_amount as refund_amount
				from ospos_migrate_sales a
				join ospos_migrate_payments b on a.sale_id = b.sale_id
				where total_payments > trans_amount and number_payments = 1
			)'
		);

		// Update existing cash transactions with refund amount
		$this->db->query('update ospos_sales_payments a
			set cash_refund =
			(select refund_amount 
				from ospos_migrate_refund b 
		...
		$this->db->query('insert into ospos_sales_payments
		   (sale_id, payment_type, employee_id, payment_time, payment_amount, cash_refund) 
			...
			join ospos_migrate_sales s on r.sale_id = s.sale_id
